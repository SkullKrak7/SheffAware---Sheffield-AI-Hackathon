...
counts = joined.groupby("grid_id").size().reset_index(name="tree_count")
grid = grid.merge(counts, on="grid_id", how="left").fillna({"tree_count": 0})
grid["tree_count"] = grid["tree_count"].astype(int)
grid[["grid_id", "tree_count"]].to_csv("tree_count_per_grid.csv", index=False)
