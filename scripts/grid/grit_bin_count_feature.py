...
counts = joined.groupby("grid_id").size().reset_index(name="grit_bin_count")
grid = grid.merge(counts, on="grid_id", how="left").fillna({"grit_bin_count": 0})
grid["grit_bin_count"] = grid["grit_bin_count"].astype(int)
grid[["grid_id", "grit_bin_count"]].to_csv("grit_bin_count_per_grid.csv", index=False)
