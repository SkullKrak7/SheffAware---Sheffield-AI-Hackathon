...
counts = joined.groupby("grid_id").size().reset_index(name="library_count")
grid = grid.merge(counts, on="grid_id", how="left").fillna({"library_count": 0})
grid["library_count"] = grid["library_count"].astype(int)
grid[["grid_id", "library_count"]].to_csv("library_count_per_grid.csv", index=False)
