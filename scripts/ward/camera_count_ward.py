...
counts = joined.groupby("admin_name").size().reset_index(name="camera_count")
wards = wards.merge(counts, on="admin_name", how="left")
wards["camera_count"] = wards["camera_count"].fillna(0).astype(int)
wards[["admin_name", "camera_count"]].to_csv("data/ward/camera_count_per_ward.csv", index=False)
